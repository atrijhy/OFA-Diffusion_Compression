...
class ImageNet(DatasetFactory):
    def __init__(self, path, resolution, random_crop=False, random_flip=True):
        ...
        class_names = [os.path.basename(os.path.dirname(p)) if '_' not in os.path.basename(p) else os.path.basename(p).split("_")[0] for p in train_files]
...
# ── Compatibility shim for accelerate ────────────────────────────────────────
# accelerate.data_loader does `from datasets import IterableDataset`.
# When it finds this local datasets.py instead of HuggingFace's package,
# the import fails. Expose a dummy so accelerate's isinstance check works
# (it will return False for all our torch datasets, which is correct).
